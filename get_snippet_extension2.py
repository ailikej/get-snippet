	...
	def get_snippet(self, kw1, kw2):
		...
		for right in range(start + 1, len(self.wordList)):
			...
			if self.wordList[right] == kwSecondFound:
				...
				left = right
		return ' '.join(self.wordListCopy[max(0,substringIndices[0]-3):substringIndices[1]+4])    # extension 2 code changes
...
# Sample test case
s1 = DecsriptionSearch("I know someone needs help with HTML and CSS. I am okay with that.")
s2 = DecsriptionSearch("I know someone needs help with HTML and CSS please")
print(s1.get_snippet("with", "CSS"))
print(s2.get_snippet("with", "CSS"))
